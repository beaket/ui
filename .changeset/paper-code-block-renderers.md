...
- **Signature:** `(code, el, ctx: { colorScheme: "light" | "dark" }) => void | Promise<void>`. The renderer replaces `el`'s content (sync or async). **Throw or reject ⇒ paper shows error text** in the widget — one path for both syntax errors and an "npm install …" install hint (the text is the consumer's; the display is paper's).
