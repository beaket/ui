...
| `nested-interactive` in `Overview/AllComponents`                                                                         | `DataTable`'s `onRowClick` makes each row a `role="button"` around the row's own checkbox. The defect is in that prop's markup; this page is only the first scan to reach it. | `@jihnma` | 2026-12-05 | #TBD          |
